...
<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between">
            ...
            <div>
                <x-button href="{{ route('master-barang.create') }}">Tambah + </x-button>
            </div>
        </div>
    </x-slot>
        ...
        <table class="min-w-full bg-white border border-gray-300 rounded">
            <thead>
                <tr class="bg-gray-100">
                    ...
                    <th class="py-2 px-4 border-b border-r">Kode Barang</th>
            ...
            <tbody>
                @forelse ($data as $item)
                    <tr class="hover:bg-gray-200 transition-all">
                        ...
                        <td class="py-2 px-4 border-b border-r">{{ $item->Nama_Barang }}</td>
                        <td class="py-2 px-4 border-b border-r">{{ $item->Satuan }}</td>
                        <td class="py-2 px-4 border-b border-r">{{ $item->Qty }}</td>
                        <td class="py-2 px-4 border-b border-r">{{ $item->Harga }}</td>
